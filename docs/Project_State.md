...
**Last verified SHAs (2026-07-21 — #53 Bundle D partial closed):**
* Backend (#53 close): code HEAD **`5604840`** — #53 demerger holding-period inheritance at the FIFO source, backend-only. Re-read the ACTUAL ledger encoding at HEAD `eb611a3f` before touching anything: SPLIT cost already correct (`_fifo_replay` divides price / scales qty via ratios, total preserved); BONUS cost already correct (real bonuses are zero-cost `type="BUY" price=0` rows, NOT the dead `type="BONUS"` branch); there is NO `DEMERGER` transaction type (Literal is `BUY/SELL/DIVIDEND/BONUS/SPLIT`; demergers are `source="manual_demerger"` BUY rows — doc-drift corrected). ONE genuine defect fixed: a demerger receipt (TMCV, JIOFIN) is a BUY dated at the RECEIPT date, so `tax_service._classify` measured the STCG/LTCG holding period from receipt rather than the parent's original acquisition (IT-Act inheritance) → could misclassify LTCG as STCG. FIX (Option A, surgical, single FIFO source of truth, NO parallel path): NEW optional `Transaction.acquired_date` (holding-period date ONLY — does NOT change cost basis (still `price`) or FIFO ordering (still `trade_date`); absent → `trade_date`, so every existing row + non-demerger BUY is byte-identical); `_fifo_replay._Lot` gains `acquired_date` (defaults to `trade_date` via `tx.get("acquired_date") or trade_date`) and the SELL disposal emits `lot.acquired_date` as `_realized_lots.buy_trade_date` — `tax_service` UNCHANGED (already reads `buy_trade_date`); `scripts/add_manual_transactions.py` TMCV + JIOFIN rows carry `acquired_date` as a PLACEHOLDER + `TODO(sahil)` to set the real earliest parent-buy date before the #42 import. 4 NEW tests (2 `test_fifo_replay`: inheritance emits inherited buy_trade_date + cost invariant + no-override defaults to trade_date; 2 `test_tax_service`: inherited date reclassifies LTCG + no-`acquired_date` regresses to STCG). Verified on EC2: 53 pytest (49 + 4), hygiene PASSED, `/health` ok/ok, `/tax/capital-gains` 200. `preview_sell`/`validate_replay` (separate dict-lot logic) + `holdings` doc + frontend all untouched; no new collection/dep/index. KNOWN LIMITATION: a single `acquired_date` carries one inherited date onto the single demerger receipt row — exact for this portfolio's single-block demergers (TMCV from one TMPV block, JIOFIN from one RELIANCE block); multi-lot pro-rata inheritance (Option B, full FIFO rewrite) deliberately out of scope. Bundle D's #68 remains OPEN. The doc commit advances HEAD further. Chat opened at backend `eb611a3f93da92a52efb81f52c5f938e2df17705`.
* Frontend (#53 close): UNCHANGED at **`3cde0d397bea87e0d875062d2075dcd6f665b111`** — #53 is backend-only.
* Backend (prior, #63/#64 close): code HEAD **`2792cefa3e85f1e77d49806f2014289f75fadd39`** — #63 + #64 shipped as one logical unit: `portfolio_service.compute_summary` extended all-holdings scan to also sum `total_dividends_received` → new `totals.total_dividends_lifetime` + `totals.total_realized_with_dividends`; `realized_pnl_lifetime` unchanged (capital-only; tax_service + /tax untouched per compliance); `routers/portfolio.py` empty-portfolio zero-block extended; `lib/api.ts` `PortfolioTotals` extended; `components/totals-row.tsx` REPLACED — re-adds "Realized P&L" StatCard (reverses #40), headline = `total_realized_with_dividends`, subtext = Capital / Dividends / N closed, grid 3→4, re-imported `TrendingUp`, tooltip links to /tax; `app/page.tsx` DashboardSkeleton 3→4. Verified on EC2: 49 pytest, hygiene PASSED, `/portfolio/summary` has both new keys, math sum true, `~/deploy-ui.sh` build+lint clean, 4-card totals row renders. No new dep, no new collection, no FIFO change. The doc commit advances HEAD further. #65 (dividend-announcement capture + drift matrix) deferred to its own design+chat.
* Frontend (#63/#64 close): code HEAD **`3cde0d397bea87e0d875062d2075dcd6f665b111`** — same unit as backend above.
...
app/
  ...
  models/
    ...
    transaction.py            Transaction. TransactionType Literal = BUY/SELL/DIVIDEND/BONUS/SPLIT (there is NO DEMERGER type — demergers are source="manual_demerger" BUY rows; #53 corrected this doc-drift). (5.6 ge=0; fix F29/F80/F82). (done #53: NEW optional `acquired_date: datetime | None = None` — the holding-period date for demerger-receipt inheritance; used ONLY for _realized_lots.buy_trade_date (STCG/LTCG classification), NOT cost basis (still `price`) or FIFO ordering (still `trade_date`); absent → trade_date, byte-identical for every existing row)
  ...
  services/
    ...
    price_service.py          EOD+intraday fetch, bulk_get_latest_prices (returns {isin: doc} with `close` Decimal128 + `date`), bulk_get_previous_closes, annotate_with_current_price, get_previous_close. IST + _to_ist() helpers (TD23). (done: #9/TD23 holiday guard; #10/TD24 docstring; #11/TD25 per-ISIN find_one; TD26 captured_at BSON Date). (#29 watchlist GET endpoints price-enrich via bulk_get_latest_prices; unchanged). (done #31 Chat B: tz-aware Mongo-write sweep -> utcnow(), incl. bulk_get_latest_intraday now_utc; in-memory aware sites annotated # tz-ok). (done #41 Chat C: NEW evaluate_stop_loss_alerts(rows) — rising-edge stop-loss breach alert on the intraday write path, first writer of alerts_log). (done #67 2026-07-08: Guard B — a first-ever fire (no prior delivered alert) now REQUIRES proof of arming (some intraday tick at/above stop_loss, reusing the existing intraday >= stop_loss lookup); suppresses fires on garbage stops set above every observed price (GAEL 1000, OLAELEC 212312) while genuine stops below price (TATASTEEL 123) are unaffected; #41 was firing correctly on polluted data.)
    holdings_service.py       recompute_holding (per-ISIN advisory-lock wrapper) + _recompute_holding_impl + _per_isin_recompute_lock (CM), validate_replay, preview_sell, _fifo_replay, _to_decimal. (done: #8/TD20 serialized per-ISIN via recompute_locks + 60s TTL). (Chat B #33: _fifo_replay + validate_replay are pure/hermetic and unit-tested directly; preview_sell + recompute_holding idempotency unit-tested against the fake_db — recompute run twice asserts stable aggregates + preserved created_at + exactly one active doc; full-exit -> None + zero active docs). (#39 Chat 9: _fifo_replay ALSO emits a per-disposal `_realized_lots` list — buy/sell trade_date + fee-normalized per-share cost/proceeds captured in the SELL branch — for tax_service to read READ-ONLY; the single FIFO source of truth, NO parallel path. _recompute_holding_impl pops `_realized_lots` off `computed` so it never lands on the extra=forbid holdings doc). (done #53: `_Lot` carries `acquired_date` (BUY branch defaults it to `trade_date` via `tx.get("acquired_date") or trade_date`); the SELL disposal emits `lot.acquired_date` as `_realized_lots.buy_trade_date` so tax_service measures the holding period from a demerger receipt's inherited parent-acquisition date. FIFO ordering + cost basis + the holdings doc are UNAFFECTED; preview_sell/validate_replay use separate dict-lot logic and were NOT touched)
    portfolio_service.py      compute_summary + _annotate_holdings + compute_risk_summary (F12/#28). _to_dec helper imported by routers/portfolio.py for by-tag totals. (#63 2026-07-21: compute_summary's all-holdings scan now ALSO sums total_dividends_received -> totals.total_dividends_lifetime + totals.total_realized_with_dividends; realized_pnl_lifetime stays capital-only; tax_service + /tax untouched per compliance).
...
scripts/
  ...
  add_manual_transactions.py    (done: #5 validate_replay on manual SELL path). (done #31 Chat B: tz-aware Mongo-write sweep -> utcnow()). (done #53: the two source="manual_demerger" rows (TMCV, JIOFIN) carry `acquired_date` = the parent's original acquisition date for holding-period inheritance; currently a PLACEHOLDER date + `TODO(sahil)` to set the real earliest parent-buy date (from the ICICI order book) before the #42 GO-LIVE import)
  ...
  cron_health_check.py          F4 daily 21:00 IST; dual-transport. (done: #23/TD38; #24/TD39). (done #66 2026-07-08: POSITIVE daily heartbeat added to the healthy branch — unconditional, no crontab/argparse change; low-priority ntfy `errors` + Resend email listing the N expected crons healthy today; anomaly + self-failure paths + own heartbeat write byte-unchanged; best-effort, never raises. Silence since 2026-06-14 was CORRECT — the old daily mail was the #47/TD22 false-positive.)
...
tests/
  ...
  test_fifo_replay.py           NEW (#33 Chat B). 7 tests of holdings_service._fifo_replay (pure): buy, partial sell P&L, full exit, buy fees fold into invested, dividend accrues, SPLIT scales qty/price, BONUS dilutes cost. (done #53: +2 — a normal BUY (no acquired_date) emits buy_trade_date == trade_date; a demerger receipt carrying acquired_date emits that inherited date as buy_trade_date while buy cost stays the receipt price.)
  ...
  test_tax_service.py           NEW (#39 Chat 9). 21 tests of tax_service (fake_db + tx() factory): FY parse (valid / 6 bad shapes / century rollover), strict >12-calendar-month classification, fee normalization into cost/proceeds, FY-window inclusive/exclusive edges, single-sell-spans-two-buy-lots STCG+LTCG split, partial sell, demerger cost read-from-ledger (not double-counted), empty, default-FY. (done #53: +2 — a manual_demerger receipt with an inherited `acquired_date` classifies LTCG (inherited date crosses 12 months) with cost basis unchanged; a manual_demerger receipt WITHOUT acquired_date regresses to STCG off the receipt date.) 53 total with the #33 + #39 harness.
docs/
...
components/
  ...
  totals-row.tsx              (Chat 9/#40: grid 4->3, removed Realized P&L card). (#63/#64 2026-07-21: REPLACED — grid 3->4, re-added Realized P&L card; headline=total_realized_with_dividends, subtext=Capital/Dividends/N-closed split; tooltip; re-imported TrendingUp)
...
**Future:** none pending. F11 read-only reformatter (Chat 9/#39). (#29 F13 watchlist needed NO new collection — it reuses monitored_stocks status="watchlist".) (#53 2026-07-21 added the optional `Transaction.acquired_date` field for demerger holding-period inheritance — no new collection/index.)
...
10. Capital-gains tax view (F11/#39) is READ-ONLY on Phase 1: tax_service sources realized lots by replaying `transactions` through holdings_service._fifo_replay (the single FIFO source of truth — extended to emit per-disposal `_realized_lots`), NEVER a parallel FIFO path, and NEVER re-applies `cost_basis_adjustments` (the §49(2C) apportioned cost is already baked into the `manual_demerger` BUY rows — re-applying would double-count). The backend `holdings.realized_pnl` field STAYS even after #40 hides it in the UI, because FIFO + the capital-gains view both read it. (Chat 9.) **#53 (2026-07-21): a demerger receipt's HOLDING PERIOD inherits the parent's original acquisition date via the optional `Transaction.acquired_date`, threaded through `_fifo_replay._Lot.acquired_date` into `_realized_lots.buy_trade_date`. The COST-BASIS invariant is unchanged (still the receipt's apportioned §49(2C) price); only the STCG/LTCG classification date moves. SPLIT/BONUS cost were already correct in the actual ledger (SPLIT preserves total cost via ratios; BONUS is a zero-cost BUY lot) and were deliberately NOT changed. There is NO `DEMERGER` transaction type.**
...
| 10.5-fix | 2026-07-08 | #66 + #67 (Phase 10.5 round-1 ops bugs, backend-only). #66 9pm heartbeat silence — cron_health_check is anomaly-only by design; pre-2026-06-14 mail was the #47/TD22 false-positive (fixed 2026-06-14); added a POSITIVE daily heartbeat to the healthy branch of `scripts/cron_health_check.py` (unconditional, no crontab/argparse change) — low-priority ntfy `errors` + Resend email listing the N healthy crons; anomaly + self-failure paths + own heartbeat write byte-unchanged; best-effort. #67 spurious stop-loss fires — #41 evaluator fired correctly on garbage `stop_loss` (GAEL 1000, OLAELEC 212312; no RELIANCE holding/alert in DB); added Guard B in `price_service.evaluate_stop_loss_alerts` requiring a prior arm tick (intraday >= stop_loss) before a first-ever fire; TATASTEEL unaffected. Verified on EC2 (49 pytest; live email+ntfy; arm-proof GAEL/OLAELEC=False; stop_loss_hit before==after==2). GO-LIVE (#42) blocking set drops #66/#67. | backend `beaadf70` / frontend `a4b27bd7` (unchanged) |
| 10.5-A | 2026-07-08 | #58 + #59 + #62 (Phase 10.5 "blocking-set chat bundles" Bundle A — frontend nav & shell, frontend-only). #58 grouped the 6 top-nav links into two dropdowns (Research: Suggestions/Watchlist/Tags; Manage: Transactions/Audit/Tax), utilities inline (9->5 controls), reused the dropdown-menu primitive, no new dep — `app/page.tsx`. #59 added a standard "Back to portfolio" (-> /) ghost link to `app/transactions/audit/page.tsx` alongside "Back to transactions". #62 NEW `components/collapsible-section.tsx` + wrapped the four dashboard sections (Portfolio totals, Allocation & movers, Risk, Holdings) in default-expanded per-session show/hide toggles. Verified on EC2 (deploy-ui build+lint clean). GO-LIVE (#42) blocking set drops #58/#59/#62. | backend `beaadf70` (unchanged) / frontend `e3a9a44b3aaf1597edeeca7500779c0eb2de3361` |
| 10.5-B partial | 2026-07-21 | #63 + #64 (Phase 10.5 Bundle B partial — dividends into realised P&L + re-show the figure, backend + frontend). #63: `portfolio_service.compute_summary` all-holdings scan extended to ALSO sum `total_dividends_received` → `totals.total_dividends_lifetime` + `totals.total_realized_with_dividends` (COMPLIANCE: dividends are income from other sources, NOT capital gains; `tax_service`/`/tax` untouched; `realized_pnl_lifetime` stays capital-only; `_fifo_replay` already persisted `total_dividends_received` per holding — no FIFO change needed); `routers/portfolio.py` empty-portfolio zero-block extended; `lib/api.ts` `PortfolioTotals` extended. #64: `components/totals-row.tsx` REPLACED — re-adds "Realized P&L" StatCard (reverses #40), headline = `total_realized_with_dividends`, subtext = `Capital: … / Dividends: … · N closed` (compliance split visible), grid 3→4, re-imported `TrendingUp`, tooltip explains dividends-NOT-capital-gains + links to /tax; `app/page.tsx` DashboardSkeleton 3→4. No new dep, no new collection, no FIFO change. Verified on EC2: 49 pytest, hygiene PASSED, `/portfolio/summary` has both new keys, math sum `capital + dividends == combined` assertion true, `~/deploy-ui.sh` build+lint clean, 4-card totals row renders. #65 (dividend-announcement capture + drift matrix) deferred to its own design+chat. GO-LIVE (#42) blocking set drops #63/#64. | backend `2792cefa3e85f1e77d49806f2014289f75fadd39` / frontend `3cde0d397bea87e0d875062d2075dcd6f665b111` |
| 10.5-D partial | 2026-07-21 | #53 (Phase 10.5 Bundle D partial — demerger holding-period inheritance at the FIFO source, BACKEND-ONLY). Investigation-first re-read of the actual ledger encoding at HEAD `eb611a3f` scoped the broadened-#53 premise DOWN: SPLIT cost already correct (`_fifo_replay` divides price / scales qty via `corporate_action` ratios, total preserved — TATASTEEL 1:10); BONUS cost already correct (real bonuses are zero-cost `type="BUY" price=0` rows — RELIANCE/ASHOKLEY/CONCOR/BPCL — NOT the dead `type="BONUS"` branch), and bonus holding-period-from-allotment is already the IT-Act treatment; NO `DEMERGER` transaction type exists (Literal is BUY/SELL/DIVIDEND/BONUS/SPLIT; demergers are `source="manual_demerger"` BUY rows — doc-drift corrected). ONE genuine defect fixed: demerger receipts (TMCV, JIOFIN) are BUYs dated at the RECEIPT date, so `tax_service._classify` measured the holding period from receipt instead of the parent's original acquisition (IT-Act inheritance), risking an LTCG→STCG misclassification. FIX (Option A, single FIFO source of truth, no parallel path): NEW optional `Transaction.acquired_date` (holding-period date ONLY; does NOT change cost basis or FIFO ordering; absent → `trade_date`, byte-identical for existing rows); `_fifo_replay._Lot.acquired_date` (defaults to `trade_date`) emitted as `_realized_lots.buy_trade_date` in the SELL branch — `tax_service` UNCHANGED (already reads `buy_trade_date`); `scripts/add_manual_transactions.py` TMCV + JIOFIN carry `acquired_date` (PLACEHOLDER + `TODO(sahil)` for the real earliest parent-buy date before #42); 4 new tests (2 fifo_replay + 2 tax_service). `preview_sell`/`validate_replay`/holdings-doc/frontend all untouched; no new collection/dep/index. Verified on EC2: 53 pytest (49 + 4), hygiene PASSED, `/health` ok/ok, `/tax/capital-gains` 200. KNOWN LIMITATION: single inherited date per receipt row — exact for this portfolio's single-block demergers; multi-lot pro-rata inheritance (Option B, full FIFO rewrite) out of scope. Bundle D's #68 remains OPEN. GO-LIVE (#42) blocking set drops #53. | backend `5604840` / frontend `3cde0d39` (unchanged) |
...
| 10.5 | #52 | USER ACCEPTANCE REVIEW (user walks the whole tool) | ROUND 1 RUN 2026-07-07 — 16 findings filed #54-#69 (Source User review); #69 = round-2 review; #66 + #67 SHIPPED 2026-07-08 (backend `beaadf70`); #58 + #59 + #62 SHIPPED 2026-07-08 (Bundle A, frontend `e3a9a44b`); #63 + #64 SHIPPED 2026-07-21 (backend `2792cefa`, frontend `3cde0d39`); #53 SHIPPED 2026-07-21 (backend, demerger holding-period at the FIFO source); second-to-last; gates GO LIVE |
| 11 | #42 | Chat 10 GO LIVE (F7 real data import) | OPEN — gated behind Phase 10.5 |
| 12 | #43-45 | Deferred TDs (TD1, TD3, TD7) | DEFERRED |
| — | #46-69 | TD21 scheduler (OPEN), TD22 (SHIPPED A), TD36 (SHIPPED A), TD40 (SHIPPED A), #50 news entity mis-tagging (OPEN, Chat 6), #51 dividend_yield ×100 (OPEN, Chat 6), #53 demerger holding-period inheritance at the FIFO source (SHIPPED 2026-07-21 — SPLIT/BONUS cost already correct, no DEMERGER type, `acquired_date` field), #54-#69 user-review findings (OPEN, Phase 10.5) | #46/#50/#51 OPEN; #54-#69 OPEN except #58/#59/#62 + #66/#67 SHIPPED 2026-07-08 and #63/#64 + #53 SHIPPED 2026-07-21 (only #60/#61 non-blocking); #47/#48/#49 SHIPPED |
...
* **#53 (Chat 9 → SHIPPED 2026-07-21):** demerger holding-period inheritance at the FIFO source — a demerger receipt now inherits the parent's original acquisition date for STCG/LTCG classification via the optional `Transaction.acquired_date`, threaded through `_fifo_replay._Lot.acquired_date` into `_realized_lots.buy_trade_date` (cost basis + FIFO ordering unchanged; `tax_service` needed no change). The broadened premise was SCOPED DOWN after re-reading the actual ledger: SPLIT cost + BONUS cost are already correct (SPLIT preserves total cost via ratios; real bonuses are zero-cost BUY lots), so they were NOT touched; there is NO `DEMERGER` transaction type (doc-drift corrected). Backend-only; 4 new tests; verified on EC2 (53 pytest, hygiene PASSED, `/tax/capital-gains` 200). KNOWN LIMITATION: single inherited date per receipt row (exact for this portfolio's single-block demergers). Bundle D's #68 (data-entry/automation front-end) remains OPEN.
* **#54-#69 (Phase 10.5 / #52 round 1, filed 2026-07-07, Source User review — full detail in master_todo.md):** #54 add-new-holding entry point (not just buy-more), #55 LLM-authored hold-horizon on buy suggestions, #56 target/sell-price breach alerts (mirror of #41 on the intraday path), #57 news alerts for holdings+watchlist/flagged names (depends on #50), #58 dashboard nav declutter (SHIPPED 2026-07-08 Bundle A), #59 audit page back-to-dashboard nav (SHIPPED 2026-07-08 Bundle A), #60 multi-email + Tailscale access (doc-prep only), #61 one-time deploy script (doc-prep only), #62 collapsible dashboard sections (SHIPPED 2026-07-08 Bundle A), #63 dividends shown in realised P&L (SHIPPED 2026-07-21), #64 re-show realised P&L (SHIPPED 2026-07-21, reverses #40), #65 dividend-announcement capture + realised-P&L drift matrix (OPEN — deferred to own design+chat), #66 9pm heartbeat silent since 2026-06-14 — SHIPPED 2026-07-08 (anomaly-only by design; pre-2026-06-14 mail was the #47/TD22 false-positive fixed 2026-06-14; added a POSITIVE daily heartbeat to cron_health_check's healthy branch — ntfy errors + email, no crontab change), #67 spurious stop-loss fires on implausible stops GAEL/OLAELEC (RELIANCE not present in DB) — SHIPPED 2026-07-08 (Guard B: a first fire requires a prior arm tick at/above stop_loss; garbage stops suppressed; #41 was firing correctly on polluted data), #68 automate corporate-action processing (demerger/split/bonus, e.g. TMPV/TMCV), #69 second user-acceptance review round. ALL BLOCKING except #60/#61. #53 demerger holding-period inheritance at the FIFO source SHIPPED 2026-07-21 (backend; `acquired_date` field); #70 filed as the #53-follow-up DATA ACTION (replace the two demerger `acquired_date` PLACEHOLDER dates with the real earliest TMPV / RELIANCE BUY dates before the #42 import; exact steps in master_todo #70). GO-LIVE (#42) blocking set (updated 2026-07-21, #53 + #63/#64 cleared): #54, #55, #56, #57, #65, #68, #70, #69 (#69 round-2 review still gates #42).
* **Phase 10.5 blocking-set chat bundles (Chat #54 overlay — full table in master_todo.md "Phase 10.5 blocking-set chat bundles"):** groups the remaining round-1 blockers for chat efficiency without renumbering. **Bundle A** #58/#59/#62 (frontend nav & shell) — SHIPPED 2026-07-08 (frontend `e3a9a44b`). **Bundle B** #63/#64/#65 (dividends & realised-P&L) — #63/#64 SHIPPED 2026-07-21 (backend `2792cefa`, frontend `3cde0d39`); #65 deferred to own design+chat. **Bundle C** #56 (clean #41 intraday mirror) then #50 -> #57 (#57 gated on #50). **Bundle D** #53 (SHIPPED 2026-07-21 — demerger holding-period at the FIFO source) + #68 (data-entry/automation front-end, OPEN). Standalones #54 (add-a-holding) and #55 (LLM hold-horizon). **#69 round-2 review LAST — gates #42.** Recommended order: A(done) -> B(#63/#64 done, #65 own chat) -> #53(done) -> C -> #68 -> #54 -> #55 -> #69 -> #42. Bundles never override a gating dependency.
...
| — | #52 | USER ACCEPTANCE REVIEW (NEW Chat 8, user request) — complete end-to-end user review/UAT; file findings as new rows; second-to-last stage; gates GO LIVE (#42). | ROUND 1 RUN 2026-07-07 — 16 findings filed #54-#69 |
| — | #54-69 | User-review findings (Phase 10.5 round 1): #54 add-new-holding entry, #55 LLM hold-horizon, #56 target/sell-price alerts, #57 news alerts (holdings+watchlist/flagged), #58 nav declutter, #59 audit back-nav, #60 multi-email/Tailscale (doc-prep), #61 one-time deploy script (doc-prep), #62 collapsible dashboard sections, #63 dividends in realised P&L (NOT capital-gains), #64 re-show realised P&L (reverses #40), #65 dividend-announcement capture + drift matrix, #66 9pm heartbeat silent since 2026-06-14, #67 spurious stop-loss fires (GAEL/OLAELEC/RELIANCE — implausible stops), #68 automate corporate-action processing, #69 second user-review round. Source User review; NO renumber. #58 + #59 + #62 SHIPPED 2026-07-08 (Bundle A, frontend `e3a9a44b`); #66 + #67 SHIPPED 2026-07-08 (backend `beaadf70`); #63 + #64 SHIPPED 2026-07-21 (backend `2792cefa`, frontend `3cde0d39`). | OPEN — all BLOCKING except #60/#61 (#58/#59/#62 + #66/#67 + #63/#64 now SHIPPED, plus #53 demerger holding-period at the FIFO source SHIPPED 2026-07-21); source of truth master_todo.md |
...
Notes: F11 (capital-gains pack, Chat 9/#39) SHIPPED 2026-07-01 — lives in `app/routers/tax.py` + `app/services/tax_service.py` + frontend `app/tax/page.tsx`; `holdings_service._fifo_replay` was extended to emit per-disposal `_realized_lots` (the read-only source tax_service reads). Its known demerger holding-period limitation (#53) was SHIPPED 2026-07-21 — a demerger receipt now inherits the parent's acquisition date for STCG/LTCG via the optional `Transaction.acquired_date` threaded through `_fifo_replay._Lot.acquired_date` into `_realized_lots.buy_trade_date`; SPLIT/BONUS cost were confirmed already-correct in the actual ledger (untouched), and there is NO `DEMERGER` transaction type. F15 (tag views, Chat 7/#28) lives in `routers/portfolio.py` + the frontend `/tags` page. F13 (watchlist) is now SHIPPED via #29 — the in-code `# F13` comments on `models/monitored_stock.py` remain scaffolding markers; the implementation carries its own #29 references. Feature-F rows for colliding numbers are for disambiguation only.
...
* (2026-07-21 #63/#64) Bundle B partial: #63 + #64 shipped together as one logical unit (backend + frontend), design proposal approved before code. **Folded dividends into realised P&L totals** (over a new separate `dividends` total only) — the combined `total_realized_with_dividends` is the headline "total money made" figure that matters to the user; `realized_pnl_lifetime` stays capital-only so the tax view and future capital-gains consumers are unaffected. **No FIFO change needed** — `_fifo_replay` already computed and persisted `total_dividends_received` per holding; the only change is aggregating it in `compute_summary`'s existing all-holdings scan (one extra field in one existing loop, zero new collections, zero new queries). **Compliance split in the UI subtext** (over hiding the breakdown) — the Capital / Dividends decomposition keeps the compliance guardrail visible; the tooltip reinforces that dividends are income-from-other-sources, not capital gains, and links to /tax. **`realized_pnl_lifetime` unchanged** (over renaming it) — several consumers (tax_service, existing frontend code) depend on the existing key; adding new additive keys is safer than renaming. **#65 deferred** (dividend-announcement capture + yfinance corp-actions + reconciliation drift matrix) — scope is materially larger (new data-capture path, possibly a new cron, reconciliation UI extension); keeping #63/#64 separate avoids conflating the trivial aggregation change with the complex capture problem.
