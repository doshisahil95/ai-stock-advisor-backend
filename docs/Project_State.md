...
scripts/
  ...
  cron_health_check.py          F4 daily 21:00 IST; dual-transport (commit 8). Confirmed healthy 5.9. Email leg flows through notify.email() (TD34 retry).
                              master_todo #24: try/except around Mongo reads; #23: read fallback log
...
* **cron_heartbeats** (F4) — Fields: cron_name, started_at, finished_at, status, error, metadata, _schema_version. **INVARIANTS:** append-only, best-effort (master_todo #23: fallback log on insert failure); `_Heartbeat.meta` is an ATTRIBUTE (`ctx.meta = {...}`). 5.9 TD14: Sunday run writes cron_name="weekly_suggestions" (NOT run_weekly_suggestions); CRON_REGISTRY matches. 5.12 TD27: purge writes cron_name="purge_news_bodies". Indexes: (cron_name, started_at desc), (started_at desc), TTL on started_at (60 days).
