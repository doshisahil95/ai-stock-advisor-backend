...
```
(none — #39 shipped; #40/#41 are frontend-led + a holdings.stop_loss reader/writer/alert, not new endpoints)
```
...
| C | 2026-07-03 | #40 realized-P&L UI hide (Phase 10, Chat C). Frontend-only — removed the "Realized P&L" StatCard from `components/totals-row.tsx` (grid 4->3, dropped now-unused `TrendingUp` import) + shrank `app/page.tsx` DashboardSkeleton 4->3. Backend `holdings.realized_pnl` field STAYS (FIFO + #39 capital-gains read it). `grep -rn realized` confirmed no other passive realized figure renders; `/tax`, sell-sheet preview/toast, transactions/audit copy KEPT. Verified via `~/deploy-ui.sh` (npm build + lint clean). | frontend `16fab5ae` |
...
| 10 | #39-41 | Chat 9 pre-launch cleanup | #39 SHIPPED 2026-07-01 (Chat 9); #40 SHIPPED 2026-07-03 (Chat C); #41 OPEN (Chat C) |
...
**Chat-bundle overlay (added 5.19, source of truth = master_todo.md "Chat bundles").** Remaining OPEN rows are grouped (NOT renumbered) into chats: **Chat A** (COMPLETE), **Chat 6** (COMPLETE), **Chat 7** (COMPLETE), **Chat 8** (COMPLETE), **Chat B** (#30, #31, #32 SHIPPED 2026-06-15; #33 SHIPPED 2026-06-26; #36, #37, #38 SHIPPED 2026-07-01 — COMPLETE), **Chat C** (#40, #41 — NEXT), **Chat D** (#43, #44, #45), and standalone large items kept one-per-chat: #39 (Chat 9 — SHIPPED 2026-07-01), **#52 (User Review chat — NEW Chat 8, second-to-last)**, #42 (Chat 10 GO LIVE, gated behind #52), #46 (scheduler). Bundles never override a per-row gating dependency.

**Open items carried past Chat B** (tracked in master_todo.md; pointer now at #41; #40 realized-P&L UI hide SHIPPED 2026-07-03 Chat C):
...
* **#40 + #41 (Chat C — NEXT):** #40 realized-P&L UI hide SHIPPED 2026-07-03 (Chat C, frontend-only, `16fab5ae`); #41 stop_loss wiring OPEN. (#39 F11 capital-gains SHIPPED 2026-07-01.)
