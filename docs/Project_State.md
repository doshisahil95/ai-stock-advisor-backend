...
**Last verified SHAs (Chat 5.19 closed, 2026-06-14):**
* Backend: **`7fcda9eb1b0e7a5cee7aba684ef7129d77b9b977`** (5.19 code HEAD after ONE commit — TD39/#24 `cron_health_check.main` self-failure dual-transport alert wrapping the per-cron Mongo reads in `scripts/cron_health_check.py`; HEAD advances after the 5.19 doc commit — pin next chat). Opened at `247510f105969f991cf661eabada2b25d47ce799` (the 5.18 doc commit). Backend-only chat.
* Frontend: **`f59958015b8b07b6e84e3add7b4a302d32b43490`** (unchanged since 5.13 — chats 5.14/5.15/5.16/5.17/5.18/5.19 backend-only).
* Prior code-HEAD closes: 5.18 `0515fef` (TD38 fallback heartbeat log + dual-source health check) · 5.17 `1d627d7` (TD37 reject NaN in _to_decimal) · 5.16 `f4168b3` (TD35 explicit inserted_id flow) · 5.15 `7d77b9c` (TD34 notify retry) · 5.14 `4ac2c95` (TD33 atomic Tavily) · 5.13 backend `090d96c` (TD29/31/32), frontend `f59958` (TD28) · 5.12 `49bf33f` (TD26 then TD27) · 5.11 `a2806cd` (TD23/24/25) · 5.10 `b34721e` (final of 5 commits `17f9f94`→TD18→`5cf3087`→`fb23307`→`b34721e`).

## Section 5: Backend file map

Layout under `app/` and top-level (verified against tree at SHA `ce5e746`; subsequently touched files tagged with the chat/TD that changed them — pending `master_todo #N` notes are live work).
...
app/
  ...
  models/
    ...
    monitored_stock.py        MonitoredStock + MonitoredStockFeedbackPatch. (A1 Literal aligned). (TD1/#43 deferred: direction-aware)
    macro_signal.py           placeholder
    conversation.py           placeholder (Chat 6 / #27)
  ...
  routers/
    ...
    suggestions.py            /suggestions/latest, /runs, /runs/{id}, /performance, /{isin}/feedback, /{isin}/audit, /feedback/audit/recent. F2 ?direction; A1 MonitoredStockFeedbackPatch; A19 Query() pattern=. (done: #17/TD31 ISIN pattern=r"^[A-Z0-9]{12}$" on the two /{isin} Path params, lines 240+260). master_todo #26: direction-aware feedback relabel
  ...
  services/
    ...
    reconciliation.py         take_auto_snapshot, drift detection, _send_drift_alerts (ntfy + email). (fix F1/F23). A2 part2 branches on notify.email() result["ok"] (unchanged after TD34 retry). master_todo #25: fire ntfy push on threshold drift; #31: tz-aware sweep (lines 78, ~138)
    ...
    outcome_tracker.py        create_outcomes_for_run, snapshot_open_outcomes, compute_system_performance. F2 direction stamp + read-time sign-flip
...
docs/
  ...
  Project_State.md              THIS FILE (5.19 doc commit; recovered from 5.8 truncation in 5.9 — Section 18 TD15)
...
uv.lock
README.md                       (5 deliverable; 5.5 §8/§11/§5). STALE: Tavily "monthly" wording wrong (code is DAILY) — master_todo #48 / TD36, not yet fixed
```

## Section 6: Frontend file map

Verified against tree at SHA `4f31b49` (unchanged 5.10–5.12; 5.13 touched notes-panel.tsx + refresh-button.tsx → frontend HEAD `f59958`; 5.14/5.15/5.16/5.17/5.18/5.19 backend-only).
...
POST   /reconciliation/auto-snapshot                 ReconciliationSnapshot (cron)   (master_todo #25: ntfy on threshold drift)
...
POST   /suggestions/{isin}/feedback                  {isin, action, status, previous_status}   (#17 ISIN pattern; #26 direction-aware relabel pending)
...
```
POST   /chat/suggestions          (F1 / Chat 6 / #27)
POST   /chat/holdings/{isin}       (F3 / Chat 6 / #27)
...
**Coverage notes:** TD33 Tavily guard has no HTTP surface (only the Sunday 06:30 fetch_news_for_universe.py path) → regression-covered at deploy via import graph + boot. TD35 explicit-id flow is exercised only by the Sunday `run_weekly_suggestions.py --direction=both` path (production caller of send_combined_digest); no HTTP surface → covered at deploy via import graph + a monkeypatched harness. TD38 (#23) fallback heartbeat path likewise has no HTTP surface → covered at deploy via landed-greps + import graph + a temp-file harness with a forced-Mongo-failure tripwire (writer fires only on insert failure; reader counts today/matching-cron only; missing-file→zeros; unwritable→swallowed; merge sums both sources) + the live `cron_health_check.py` running end-to-end through the merge loop. TD39 (#24) self-failure path likewise has no HTTP surface → covered at deploy via landed-greps + a hermetic tripwire (stub `cron_run` + both transports + a raising `count_today_heartbeats` → assert one ntfy + one email + the original error re-raised, NO real sends) + a live-transport variant (stub only the Mongo read + `cron_run`, transports real → confirm the alert physically lands) + the live happy-path run exiting 0.

**Open scheduling work:** TD21/#46 registry-generated crontab migration (deferred; CRON_REGISTRY gains a parseable cron expr → `scripts/render_crontab.py` renders a committed `ops/crontab` → deploy.sh installs it + a drift-validation step. Chosen over in-process APScheduler — on the 1 GB t3.micro the ~5-min Sunday dossier run would compete with the live API and die on every restart. Its own dedicated chat). TD22/#47: track_suggestion_outcomes FAILS every weekday (19:45 IST, 0 success / 1 failure) — fires the 21:00 health email daily; root-cause pending. TD40/#49: weekly_suggestions_sell idle CronSpec `expected_weekdays={6}` → false Sunday MISSING anomaly (see CRON_REGISTRY note above); backend-only fix.
...
* **Tavily:** TAVILY_API_KEY (req) · TAVILY_DAILY_CALL_LIMIT (default 200) — hard ceiling on calls_today per UTC day, enforced atomically (TD33); DAILY resets 00:00 UTC (README/data_flow "monthly" prose stale → #48/TD36) · TAVILY_SEARCH_DEPTH (default "basic") · TAVILY_MAX_RESULTS_PER_QUERY (default 5).
...
6. prices_intraday writes are append-only within a day. (5.11 #9/TD23: holiday-stale bar dropped. 5.12 TD26: 90-day captured_at_ttl bounds it; works because captured_at is a BSON Date.)
...
* Snapshot eligibility for snapshot_open_outcomes is `tracking_status != "expired"` (A.5). Auto-expiry only flips "open" outcomes at day 180; user-set labels never overwritten (A.5). Feedback re-labels the MOST RECENT non-expired outcome for the ISIN (A.5.1).
...
* A1: monitored_stocks writes go through `MonitoredStockFeedbackPatch(...).model_dump(exclude_none=True)`. `extra="forbid"` catches drift.
...
* 5.13 (#17/TD31): the ISIN Path() params on GET /suggestions/{isin}/audit and POST /suggestions/{isin}/feedback carry `pattern=r"^[A-Z0-9]{12}$"` alongside min_length=12, max_length=12 → a malformed ISIN 422s at the boundary.
...
* Sell-side uses different groups (booking_opportunity/valuation_stretch/risk/tax_concentration) and gates (in_profit/min_position_age/earnings_proximity). CandidateScore has FIXED buy-side group fields; sell-side group scores flow through group_meta (TD7/#45 deferred). monitored_stocks currently direction-agnostic (TD1/#43 deferred).
...
The Chat 5.10 SellSheet recorded_with_warning follow-up remains OPEN and untouched through 5.19 (out of each phase's scope).
...
* **#26 (P2-6/TD1):** direction-aware feedback relabel — add `"direction": payload.direction` to the outcome filter in `submit_feedback`. Phase 7.
...
* **#49 (TD40, NEW 5.19):** the idle `weekly_suggestions_sell` CronSpec has `expected_weekdays={6}`, so `cron_health_check` sends a false `MISSING: weekly_suggestions_sell` anomaly alert every Sunday even though the umbrella `weekly_suggestions` `--direction=both` cron covers both directions. Fix: give the idle spec `expected_weekdays=set()` or exclude idle/umbrella-covered entries from the MISSING check. `app/services/cron_heartbeat_service.py`. Filed 5.19, not yet fixed.
...
* Cron heartbeats are best-effort WITH a disk fallback: when the Mongo insert raises, `cron_heartbeat_service._persist` appends the heartbeat as JSON-per-line to `_FALLBACK_LOG_PATH = "/home/ubuntu/cron-heartbeat-fallback.log"` via `_append_fallback` (NEVER raises — no-mask contract); `cron_health_check.main` merges Mongo + `count_today_heartbeats_from_fallback` counts before evaluating MISSING/FAILED. A run lands in at most one source, so the merge never double-counts. Do NOT make `_append_fallback` raise, and do NOT drop the Mongo path in favour of the file. The log matches the `cron-*.log` logrotate glob (no new rotation). This hardens heartbeat PERSISTENCE only — a cron whose own body throws (TD22/#47) is a real failure, correctly reported. (5.18 TD38.)
* `cron_health_check.main`'s ONLY Mongo reads are the per-cron `count_today_heartbeats` calls in the registry loop — that loop is wrapped in try/except so an unreachable Atlas fires a dedicated "anomaly: health-check itself failed" alert on BOTH transports (ntfy `push_public("errors",...)` GUARDED + `notify.email()`, both Mongo-independent) and then RE-RAISES (so `cron_run` records the run failed and its heartbeat → the #23 disk fallback) instead of returning 0. Do NOT widen the wrap to include the pure `ist_today_window_utc()`, disk-only `count_today_heartbeats_from_fallback()`, or in-memory `get_registry()` (that would let a non-Mongo bug masquerade as "Mongo unreachable"); do NOT leave the self-failure `push_public` unguarded; do NOT disturb the #23 merge loop inside the wrap; do NOT return success after the reads fail. (5.19 TD39.)
...
**Chat 5.18:** "Heartbeat errors" splits into two distinct failure modes — distinguish them before acting: (a) heartbeat PERSISTENCE failure (the Mongo insert raises; the run actually succeeded) → false MISSING, the gap #23/TD38 closes via the disk fallback; (b) the cron's OWN body throwing (recorded faithfully as `status="failure"`) → a real bug, NOT something #23 touches. A `1 failure(s) today` line PROVES the heartbeat machinery worked (the failure row reached Mongo), so it is never the persistence bug — it's TD22/#47. Mirror an existing reader/writer exactly when adding a fallback (`count_today_heartbeats_from_fallback` is a 1:1 shape-match of `count_today_heartbeats`, format lives in ONE module next to the writer). The fallback timestamp is stored ISO-8601 and must be normalized tz-aware→naive-UTC (`_parse_fallback_dt`) to compare against the naive-UTC window from `ist_today_window_utc`. A best-effort sink must swallow ALL its own errors (missing file, unwritable path, malformed line) and contribute zero — never mask the underlying cron error. Verify a no-HTTP-surface change with a temp-file harness + a forced-Mongo-failure tripwire (a `Collections` stub whose `cron_heartbeats()` RAISES) + landed-greps + the live health check running through the merge loop — not curls. A docstring that misspells its own constant (`_FALLBACK_LOG_PAT`) is cosmetic but still gets fixed before the audit trail — a user grep caught it; trust the grep.

**Chat 5.19:** `cron_health_check.main`'s ONLY Mongo reads are the per-cron `count_today_heartbeats` calls in the registry loop — wrap THAT loop (not the pure `ist_today_window_utc()` / disk-only `count_today_heartbeats_from_fallback()` / in-memory `get_registry()`) in try/except so an unreachable Atlas fires a dedicated "anomaly: health-check itself failed" alert instead of crashing silently. Mirror the normal anomaly path's dual transport (ntfy `push_public("errors",...)` + `notify.email()`, both Mongo-independent — user-confirmed over ntfy-only as literally specced), but GUARD the ntfy leg — `push_public` RAISES on failure, so a failed self-failure push must not block the email leg or mask the original Mongo error. Then RE-RAISE (don't return 0) so `cron_run` records the run as a failure (its heartbeat falls to the #23 disk fallback since Mongo is down) and tomorrow's check re-evaluates — recording success would be dishonest and would write a false-success fallback heartbeat. The #23/TD38 fallback-merge loop stays verbatim INSIDE the wrap. Verify with TWO harnesses: a hermetic tripwire (stub `cron_run` + both transports + a `count_today_heartbeats` that raises → assert one ntfy + one email + the original error re-raised, NO real sends) AND a live variant (stub only the Mongo read + `cron_run`, leave the transports real → confirm the alert physically lands). A health-check self-failure alert (Mongo unreachable) and a normal anomaly alert (e.g. the Sunday `weekly_suggestions_sell` false MISSING, #49) are DIFFERENT alerts — and a test harness's stubbed `[stub] push_public` / `[stub] email` lines are NOT missing real deliveries, they ARE the stubs firing (no network).
...
* (5.18) Conflating a heartbeat PERSISTENCE failure (false MISSING; #23 fixes) with a cron whose OWN body fails (real bug; TD22 — out of scope); assuming a "fix the heartbeat error" item will silence a `1 failure(s)` alert (it won't — that's a recorded real failure); making a best-effort fallback sink raise instead of swallowing its own errors; comparing an ISO fallback timestamp to a naive-UTC window without normalizing tz; verifying a no-HTTP-surface fallback path without a forced-failure tripwire (a passing happy path proves nothing about the except branch); leaving a misspelled constant in a docstring because "tests pass."
* (5.19) Returning 0 (success) from `cron_health_check.main` after the Mongo reads fail — must re-raise so the run is recorded failed (else a false-success disk-fallback heartbeat is written); leaving the self-failure ntfy unguarded so a `push_public` raise crashes the alerter before the email leg; wrapping more than the per-cron read loop (pulling pure time math / disk-fallback / in-memory registry into the try so a non-Mongo bug masquerades as "Mongo unreachable"); disturbing the #23/TD38 merge loop while re-indenting; mistaking a test harness's stubbed `[stub] push_public` / `[stub] email` lines (no network) for missing real notifications; folding the separate `weekly_suggestions_sell` Sunday false-MISSING (#49/TD40) or the `track_suggestion_outcomes` body failure (TD22/#47) into #24's scope.
...
* Gating filter on snapshot_open_outcomes → `tracking_status != "expired"`.
...
* What happens when a cron heartbeat's Mongo insert fails → `_persist` appends it to `/home/ubuntu/cron-heartbeat-fallback.log` (JSON-per-line, via `_append_fallback`, never raises); `cron_health_check.main` merges `count_today_heartbeats` + `count_today_heartbeats_from_fallback` so it's NOT a false MISSING. A run lands in one source only → no double-count (5.18 TD38).
...
* Why does cron_health_check alert MISSING: weekly_suggestions_sell every Sunday → the idle weekly_suggestions_sell CronSpec carries expected_weekdays={6}; the umbrella weekly_suggestions cron covers both directions, so it's a FALSE positive — master_todo #49/TD40, NOT a #24 regression.
* Does #23/TD38 fix the daily `track_suggestion_outcomes` failure → NO. That's the cron's own body failing (TD22/#47), recorded correctly as a real failure. TD38 only hardens heartbeat PERSISTENCE (the false-MISSING-on-Mongo-outage case); TD39 only hardens the health-check script's own resilience.
...
| TD38 | #23 | Fallback heartbeat log — `_persist` appends JSON-per-line to `/home/ubuntu/cron-heartbeat-fallback.log` on Mongo-insert failure (was `except: pass`); `count_today_heartbeats_from_fallback` mirrors the Mongo counter; `cron_health_check` merges both (no double-count). Hardens persistence only — not a failing cron body (TD22) (0515fef) | 5.18 |
| TD39 | #24 | cron_health_check.main self-failure alert — wraps the per-cron Mongo-read loop (`count_today_heartbeats`) in try/except; on failure fires "anomaly: health-check itself failed" on BOTH transports (GUARDED ntfy `push_public("errors",...)` + `notify.email()`) then RE-RAISES so the run is recorded failed (heartbeat → #23 disk fallback); pure/disk/in-memory calls left outside the wrap; #23 merge loop preserved verbatim inside it. LAST Phase 6 item → Phase 6 COMPLETE (7fcda9e) | 5.19 |

**OPEN / DEFERRED TDs (full):**

| TD | master_todo | Item | Status |
|---|---|---|---|
| TD1 | #43 | Make monitored_stocks direction-aware (add direction field, dual rows per ISIN). Reconcile with #26. | DEFERRED — post-launch |
...
| TD22 | #47 | track_suggestion_outcomes cron FAILS every weekday (19:45 IST, 0 success / 1 failure) → fires the 21:00 IST health email daily. Root-cause (read cron-outcomes.log at HEAD + the script body) and fix. | OPEN — next ops chat |
| TD36 | #48 | README.md + docs/data_flow.md say the Tavily quota is "monthly"; the code is DAILY (resets 00:00 UTC). Correct the prose. DOC-ONLY, no code. Project_State Sections 5/7/10/12/14 already correct. | OPEN — doc cleanup chat (filed 5.16, not yet fixed) |
| TD40 | #49 | Idle `weekly_suggestions_sell` CronSpec has `expected_weekdays={6}` → cron_health_check sends a false `MISSING: weekly_suggestions_sell` anomaly alert every Sunday even though the umbrella `weekly_suggestions` `--direction=both` cron covers both directions. Fix: `expected_weekdays=set()` on the idle spec or exclude idle/umbrella-covered entries from the MISSING check. Backend-only, `app/services/cron_heartbeat_service.py`. | OPEN — filed 5.19 (surfaced during #24 verification) |
...
* (5.18) #23 disk-file fallback (JSON-per-line) over a second Mongo collection / queue — the whole point is to survive Mongo being unreachable, so the sink must not depend on Mongo; chosen as the minimal evolution of the existing `except: pass`. Reader mirrors `count_today_heartbeats` 1:1 and lives in the same module as the writer (format defined once) rather than parsing in the script. Merge-counts (no double-count) over dedup logic, justified by the at-most-one-source invariant. No new logrotate (reuses the `cron-*.log` glob). Scoped to persistence only; #24 (health-check self-failure) and TD22 (failing cron body) explicitly left separate. Verified via temp-file + forced-failure tripwire harness over a live Mongo-outage simulation.
* (5.19) #24 wrapped ONLY the per-cron `count_today_heartbeats` loop (not the pure `ist_today_window_utc()` / disk-only `count_today_heartbeats_from_fallback()` / in-memory `get_registry()`) so the self-failure path means exactly "Mongo unreachable"; dual-transport (ntfy + email) mirroring the normal anomaly path (user-confirmed) over ntfy-only as literally specced, since `notify.email()` is equally Mongo-independent and gives the same redundancy; RE-RAISE after alerting (assistant's call, user-delegated) over returning 0 — the check genuinely failed, so recording success would write a false-success disk-fallback heartbeat and hide the outage; the ntfy leg guarded because `push_public` raises. Filed the `weekly_suggestions_sell` Sunday false MISSING as #49/TD40 rather than folding it into #24 (no-renumber convention). Verified via a hermetic tripwire + a live-transport variant over a real Atlas outage.
