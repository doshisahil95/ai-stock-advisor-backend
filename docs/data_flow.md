...
| `monitored_stocks` | Stateful per-ISIN feedback state + watchlist, status (`tracking`/`passed`/`rejected`/`watchlist`) + feedback/watchlist fields. ONE doc per ISIN (both writers upsert on `{isin}`) | `routers/suggestions.submit_feedback` via `MonitoredStockFeedbackPatch` (A1); `routers/watchlist` CRUD via `MonitoredStockWatchlistPatch` (#29, status="watchlist") | Two-mechanism exclusion: `get_excluded_isins` at run-build + `_build_user_action` at serialization; `get_watchlist_isins` (status=="watchlist") for the universe + both crons (#29). NOTE: `get_excluded_isins` scans only rejected/tracking, so watchlist is never excluded — flipping rejected→watchlist un-suppresses |
