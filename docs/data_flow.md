...
> Last updated: 2026-08-07 (Chat 12 — TD1/#43 direction-scoped exclusion: `monitored_stocks` gained `feedback_direction` and `get_excluded_isins` gained a `direction` param, so a sell-side "rejected" no longer suppresses the buy side; TD7/#45 sell group scores are now first-class `CandidateScore` fields. Prior: 2026-07-28 #69 round-2 review sync — corrected the `run_weekly_suggestions` crontab line to the LIVE `--direction=both` (argparse does NOT accept `--notify`/`--run-type`), the CRON_REGISTRY count to 11, added the shipped collections/alerts, and closed the stale Q3. Earlier baseline: 2026-05-24 post-Chat-5.5 TD cleanup.)  This is a "future-self" doc — open it when you need
...
| `monitored_stocks` | Stateful per-ISIN feedback state + watchlist, status (`tracking`/`passed`/`rejected`/`watchlist`) + feedback/watchlist fields + `feedback_direction` (TD1/#43, buy/sell/None). ONE doc per ISIN (both writers upsert on `{isin}`) | `routers/suggestions.submit_feedback` via `MonitoredStockFeedbackPatch` (A1, now stamps `feedback_direction=payload.direction`); `routers/watchlist` CRUD via `MonitoredStockWatchlistPatch` (#29, status="watchlist", never touches `feedback_direction`) | Two-mechanism exclusion: `get_excluded_isins(direction=...)` at run-build (TD1/#43: direction-scoped — excludes an ISIN only when its `feedback_direction` matches the requested side, None=legacy=matches any) + `_build_user_action` at serialization (cosmetic direction leak documented-deferred); `get_watchlist_isins` (status=="watchlist") for the universe + both crons (#29). NOTE: `get_excluded_isins` scans only rejected/tracking, so watchlist is never excluded — flipping rejected→watchlist un-suppresses |
...
The buy-side CANDIDATE universe is NIFTY 100 ∪ watchlist (F13/#29 — `build_universe` unions `get_watchlist_isins()`, status=="watchlist" in `monitored_stocks`, resolving watchlist ISINs outside NIFTY 100 from `instruments` by ISIN; held is filtered DOWNSTREAM by `filter_universe`, so net buy candidates = NIFTY 100 ∪ watchlist − held − excluded). The DATA-REFRESH universe (fundamentals + earnings + news) is NIFTY 100 ∪ active holdings ∪ watchlist — held + watchlist names outside NIFTY 100 still need fresh fundamentals + earnings + news (held for F2 sell-side scoring; watchlist for #29, which is why both crons fold watchlist ISINs in — the F13 data-volume multiplier).  `scripts/seed_nifty100.py` is the one-time / occasional seeder — it fetches NSE's official `ind_nifty100list.csv`, marks matched instruments with `in_nifty100=True`, and backfills 5y of price history for the matched ISINs.  `refresh_fundamentals.py` then runs Sundays at 06:00 IST against `get_nifty100_union_holdings()` (which folds in watchlist ISINs via `get_watchlist_instruments()` as of #29) and upserts one `instruments_fundamentals` doc per ISIN: ROE, ROA, operating margin, debt-to-equity, P/E, P/B, earnings growth YoY, plus the fundamentals timestamp.  Missing-data ISINs are skipped (not zeroed) so normalization isn't poisoned.
...
### Watchlist CRUD (F13 / #29)

`routers/watchlist.py` exposes `GET /watchlist` (price-enriched list), `GET /watchlist/{isin}`, `PUT /watchlist/{isin}` (idempotent create/update — 404s an unknown instrument, flips status to "watchlist" which un-excludes a previously rejected/acted ISIN, write-before-apply audit, Money→Decimal128, upsert on `{isin}`), and `DELETE /watchlist/{isin}` (hard-delete ONLY a status=="watchlist" doc, so a feedback row is never nuked). It reuses `monitored_stocks` (status="watchlist") — no new collection — and the SAME `monitored_stocks_audit` collection via `log_change` (the action Literal was widened to `AuditAction`: feedback actions + watchlist_add/update/remove). Watchlisting a name adds it to the buy-side universe AND to the weekly fundamentals + earnings + daily news crons (the data-volume multiplier; each watchlist name consumes ~1 Tavily call/run against the daily quota — bounded by the existing ceiling, a documented soft guardrail, no hard watchlist-size cap). NOTE: the browser PUT requires `PUT` in `CORSMiddleware allow_methods` (added #29) — curl-from-box has no preflight, so server-side curls passed while the browser failed until this landed.
